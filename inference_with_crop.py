...
def predict_folder(rank, world_size):
    ...
    for path_img in path_images:
        ...
        # Cropping parameters
        crop_size = 1024  # Size of each crop
        stride = 1000     # Overlap between crops

        # Create an empty tensor to hold the output
        output = torch.zeros_like(tensor)
        count_map = torch.zeros_like(tensor)
        ...
        for y in range(0, H, stride):
            for x in range(0, W, stride):
                ...
                # Pad the crop if it's smaller than crop_size
                pad_h = crop_size - (y2 - y1)
                pad_w = crop_size - (x2 - x1)
                if pad_h > 0 or pad_w > 0:
                    crop = F.pad(crop, (0, pad_w, 0, pad_h), value=0)

                # Process the crop through the model
                with torch.no_grad():
                    output_crop = model(crop, side_loss=False)

                # Remove padding from the output crop
                output_crop = output_crop[:, :, :y2 - y1, :x2 - x1]

                # Accumulate the output and count map
                output[:, :, y1:y2, x1:x2] += output_crop
